...
bool WindowManager::parseKeyBinding(
    ...
    if (action_string.rfind("exec", 0) == 0) {
        ...
        if (command == "terminal" || command == "term") binding.action = ACTION_SPAWN_TERMINAL;
        else { binding.action = ACTION_EXEC; binding.cmd = command; }
    } else if (action_string.rfind("workspace", 0) == 0) {
        std::string number = trim(action_string.substr(9));
        binding.arg = parseIntOrDefault(number, 1);
    ...
    } else if (action_string == "quit" || action_string == "exit") {
...
void WindowManager::setupKeybindings() {
    if (has_runtime_keybinds) return;
    keybindings.clear();
    for (auto &d : DEFAULT_BINDS) {
        std::string combo = d.combo;
        std::string actionStr = d.action;
        KeyBinding binding{};
        if (parseKeyBinding(combo, actionStr, binding)) keybindings.push_back(binding);
    }
}
...
void WindowManager::handleKeyPress() {
    ...
    for (auto &b : keybindings) {
        ...
        if (b.action == ACTION_SPAWN_TERMINAL) spawnTerminal();
        else if (b.action == ACTION_SWITCH_WORKSPACE) switchWorkspace(b.arg);
        else if (b.action == ACTION_MOVE_WORKSPACE) moveToWorkspace(b.arg);
        else if (b.action == ACTION_QUIT) { if (display) XCloseDisplay(display); exit(0); }
        else if (b.action == ACTION_EXEC) { if (fork() == 0) { execlp(b.cmd.c_str(), b.cmd.c_str(), nullptr); exit(1); } }
        return;
    }
}
...
KeySym WindowManager::parseKeyString(const std::string& s) {
    ...
    if (text.size() == 1) {
        ...
        if (character >= '0' && character <= '9') return XK_0 + (character - '0');
        if (character >= 'a' && character <= 'z') return XK_a + (character - 'a');
        if (character >= 'A' && character <= 'Z') return XK_A + (character - 'A');
    }
    KeySym key = XStringToKeysym(text.c_str());
    if (key == NoSymbol) {
        for (char &character : text) character = static_cast<char>(tolower(static_cast<unsigned char>(character)));
